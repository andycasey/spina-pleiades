...
model = load_stan_model("lines.stan")
...
init = dict(c=np.nanmedian(y, axis=1), m=np.zeros(S), scatter=0.001 * np.ones(A),
    ln_f=0)
...
for i in range(S):
    ...
    ax.scatter(x[ok], (y.T[i] - offsets)[ok], facecolor='k')
    ax.errorbar(x[ok], (y.T[i] - offsets)[ok], 
        yerr=total_yerr[ok], 
        fmt=None, ecolor="k")


    lines = np.dot(chains["m"][:, i].reshape((-1, 1)), np.atleast_2d(x)) \
          + chains["c"][:, i].reshape((-1, 1))
